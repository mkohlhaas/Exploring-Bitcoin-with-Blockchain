...
def parseBlockHeader(mptr: mmap, start: int, height: int):
    ...
    mptr.seek(seek)
    block_header = {}
    block_header['magic_number'] = int.from_bytes(
        mptr.read(4), byteorder='little')
    block_header['block_size'] = int.from_bytes(
        mptr.read(4), byteorder='little')
    ...
    block_header['timestamp'] = int.from_bytes(
        mptr.read(4), byteorder='little')
    ...
    while True:
        ...
        if 'data_pos' in jsonobj:
            block_filepath = os.path.join(
                blocks_path_g, 'blk%05d.dat' % jsonobj['n_file'])
            start = jsonobj['data_pos']
            print('height = %d' % jsonobj['height'])
        elif 'undo_pos' in jsonobj:
            block_filepath = os.path.join(
                blocks_path_g, 'rev%05d.dat' % jsonobj['n_file'])
            start = jsonobj['undo_pos']
        # load file to memory
        with open(block_filepath, 'rb') as block_file:
            # File is open read-only
            with mmap.mmap(block_file.fileno(), 0,
                           prot=mmap.PROT_READ,
                           flags=mmap.MAP_PRIVATE) as mptr:
                blockheader, prev_blockhash_bigendian_b = parseBlockHeader(
                    mptr, start, jsonobj['height'])
