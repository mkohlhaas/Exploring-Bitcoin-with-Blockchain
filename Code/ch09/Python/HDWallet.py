...
from cryptotools.ECDSA.secp256k1 import Point
from cryptotools.ECDSA.secp256k1 import G as secp256k1_G
from cryptotools.ECDSA.secp256k1 import N as secp256k1_n, CURVE
...
def genSeedFromStr(password: str, salt: str):
    ...
    seed = pbkdf2.pbkdf2(hashlib.sha512, password_b,
                         salt_b, iterations_g, dklen_g)
    return seed
...
def genHardenedChildPrivKey(privkey: int, chaincode: bytes, index: int):
    ...
    h = hmac.new(chaincode, b'\x00' + privkey_b +
                 index_b, hashlib.sha512).digest()
    h256 = int.from_bytes(h[0:32], byteorder='big')
    child_privkey = finiteFieldAddition(h256, privkey, secp256k1_n)
    child_chaincode = h[32:64]
    return child_privkey, child_chaincode
