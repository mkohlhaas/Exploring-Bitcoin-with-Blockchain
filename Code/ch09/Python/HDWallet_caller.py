...
from HDWallet import genSeedFromStr
from HDWallet import genMasterKeys
from HDWallet import genNormalChildPrivKey
from HDWallet import genNormalChildPubKey
from HDWallet import genNormalParentPrivKey
from HDWallet import genHardenedChildPrivKey
from HDWallet import getMasterXPrv
from HDWallet import getMasterXPub
from HDWallet import getXPrv
from HDWallet import getXPub

if __name__ == '__main__':
    mnemonic_code_l = ['moral', 'submit', 'comfort', 'cupboard', 'organ',
                       'expand', 'home', 'bid', 'dawn', 'ozone', 'omit',
                       'helmet']
    ...
    child_privkey_i, child_chaincode = genNormalChildPrivKey(
        privkey, chaincode, 1)
    ...
    child_pubkey_b, child_chaincode = genNormalChildPubKey(
    ...
    index = ((1 << 31) + 1)  # 2^31 + 1
    child_privkey_i, child_chaincode = genHardenedChildPrivKey(
    ...
    child_xprv = getXPrv(pubkey_b.hex(), child_chaincode,
                         child_privkey_i, 1, 1)
    print('child xprv:')
    print(child_xprv)

    child_xpub = getXPub(pubkey_b.hex(), child_chaincode,
                         child_pubkey_b.hex(), 1, 1)
    print('child xpub:')
    print(child_xpub)
