...
from FindTransactionInLevelDB import findTransaction
from FindTransactionInLevelDB import BLOCK_HEADER_SIZE
from FindTransactionInLevelDB import txindex_db_g
...
def getTransactionFee(tx: dict):
    ...
    for inp in tx['inputs']:
        ...
        inp_val += getTransactionOutAmount(
            prev_tx_hash,
            inp['prev_tx_out_index'], txindex_db_g)
    out_val = 0
    for out in tx['outs']:
...
def getBlockFeeReward(block_hash: bytes, block_db):
    ...
    if 'data_pos' in block_index:
        block_filepath = os.path.join(
            blocks_path_g, 'blk%05d.dat' % block_index['n_file'])
        start = block_index['data_pos']
    elif 'undo_pos' in block_index:
        block_filepath = os.path.join(
            blocks_path_g, 'rev%05d.dat' % block_index['n_file'])
